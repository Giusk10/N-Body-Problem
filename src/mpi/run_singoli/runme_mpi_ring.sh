...
# Compile
mpicc -std=c99 -o mpi_ring mpi_nBody_ring.c -lm

# Machinefile (must exist in the current directory)
MACHINEFILE="machinefile.txt"

# Output files
output_eth="mpi_ring_results_ethernet.txt"
output_ib="mpi_ring_results_infiniband.txt"
...
do
    ...
    do
        echo "=== Ring ETHERNET: $num_processes proc, $num_particles particles ==="
        mpirun $ETH_FLAGS $MAP_FLAGS -np $num_processes \
               -machinefile $MACHINEFILE ./mpi_ring $num_particles >> "$output_eth"
        echo "--------------------------------------------------------"

        echo "=== Ring INFINIBAND: $num_processes proc, $num_particles particles ==="
        mpirun $IB_FLAGS $MAP_FLAGS -np $num_processes \
               -machinefile $MACHINEFILE ./mpi_ring $num_particles >> "$output_ib"
        echo "--------------------------------------------------------"
    done
done
